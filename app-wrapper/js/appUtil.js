...
var appUtil = {
    ...
    sendReportEmail: async function(){
        ...
        var transporter = nodemailer.createTransport({
            service: appState.config.mailService,
            auth: {
                user: appState.config.smtpUsername,
                pass: appState.config.smtpPassword
            }
        });
        ...
        var messageLogContents;
        try {
            debugLogContents = JSON.stringify(JSON.parse("[" + fs.readFileSync(path.resolve(appState.config.debugMessagesFilename)) + "]"), ' ', 4);
        } catch (ex){

        }
        try {
            messageLogContents = JSON.stringify(JSON.parse("[" + fs.readFileSync(path.resolve(appState.config.userMessagesFilename)) + "]"), ' ', 4);
        } catch (ex) {

        }

        var mailOptions = {
            from: appState.config.smtpUsername,
            to: appState.config.reportMails.join(', '),
            subject: 'Mascom web_sync report',
            text: 'Mascom web_sync report for ' + new Date().toISOString(),
            html: await _appWrapper.app.sync.prepareReportHtml(),
            attachments: []
        };
        ...
        if (debugLogContents){
            filesToArchive.push(path.resolve(appState.config.debugMessagesFilename));
        }

        if (messageLogContents){
            filesToArchive.push(path.resolve(appState.config.userMessagesFilename));
        }

        if (appState.config.appConfig.logMysqlQueries){
            var queryLogPath = path.resolve(appState.config.app.queryLogFilename);
            filesToArchive.push(queryLogPath);
        }



        if (filesToArchive && filesToArchive.length){
            var archiveDir = path.dirname(path.resolve(appState.config.debugMessagesFilename));
            var zipPath = await _appWrapper.fileManager.zipFiles(path.join(archiveDir, '/report.zip'), filesToArchive);
        }

        if (zipPath && fs.existsSync(zipPath)){
            var reportContents = fs.readFileSync(zipPath);
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                this.log("Error sending email: '{1}'", "error", [error], false, this.forceDebug);
