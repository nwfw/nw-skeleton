...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    data().each(function(doclet) {
        ...
        if( doclet.meta.filename ) {
            ...
            if ( filenameArr.indexOf(filename) == -1 ) {
    ...
    statistic.files = filenameArr;
    statistic.members = data({kind:'member'}).count();
    statistic.functions = data({kind:'function'}).count();
    ...
    function testTemplate() {
        ...
        var filename = 'statistic.html';
        var testPath = path.join(outdir, filename) ;
        var html = view.render( 'test.tmpl' , dataTest );
